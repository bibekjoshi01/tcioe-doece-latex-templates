...
\begin{figure}[H]
    ...
    \caption{Audio deepfake detection service architecture}
    \label{fig:detection_service}
\end{figure}

\subsubsection{Service Architecture and Integration Layer}
...
\textbf{Key idea:} the detector is not relying solely on raw waveform patterns; it operates on representations that retain both acoustic detail and contextual speech information.
...
This response enables real-time actions such as:
\begin{itemize}
    \item UI alerts,
    \item call monitoring warnings,
    \item logging for security analysis,
    \item policy-based intervention by the RTC platform.
\end{itemize}

\subsubsection{End-to-End Detection Formulation}

The complete operation of the detection service can be summarized as
\begin{equation}
\hat{d}_t = \mathcal{C}\big(F(P(x_{out}[n]))\big),
\end{equation}
where:
\begin{itemize}
    \item \(x_{out}[n]\) is the reconstructed receiver-side audio,
    \item \(P(\cdot)\) denotes preprocessing,
    \item \(F(\cdot)\) denotes feature extraction,
    \item \(\mathcal{C}(\cdot)\) denotes classification.
\end{itemize}

This formulation reflects the complete inference pipeline used by the proposed system and defines the operational boundary between RTC audio reconstruction and deepfake decision making.